# 自动化测试框架

## 1. 介绍

基于`pytest`编写自动化测试框架，集成`allure`测试报告和飞书消息通知，支持按照测试类型（如冒烟测试）、测试平台（如OPPO），
目前使用`uiautomator2`作为UI自动化工具集，默认读取`config.yaml`文件作为配置参数，也可在`run.py`入口文件重新指定配置参数，
与`jenkins`无缝集成，实现参数更新触发自动化测试并生成报告。

## 2. 项目结构

```
autotest_frame/
├── base/                   # 基类驱动层
├── data/                   # 数据层
├── page/                   # 页面对象层
├── log/                    # 日志文件目录
├── testcase/               # 测试用例
├── util/                   # 模块方法集
├── .gitignore              # git忽略配置
├── .python-version         # python版本
├── config.yaml             # 全局配置文件
├── conftest.py             # pytest钩子函数集
├── main.bat                # 本地执行入口文件
├── pyproject.toml          # 项目依赖项 (uv)
├── pytest.ini              # pytest配置文件
├── run.py                  # CI/CD执行入口
├── run_task.py             # 测试任务执行入口脚本
├── uv.lock                 # 锁定的依赖版本 
└── uv_env.bat              # uv管理虚拟环境 
```

## 3. 环境准备

在开始之前，请确保您的系统已安装以下软件：

   **python**: 建议版本与 `.python-version` 文件中指定的版本一样。

   **java**: allure报告生成需要java环境。

## 4. 配置

4.1 **选择配置 `config.yaml`**：这是框架的核心配置文件，默认读取这个yaml文件作为配置参数。
   ```yaml
    device_id: '96a38567'    # 设备号
    test_type: 'test_smoke'  # 测试类型
    test_platform: 'VIVO'    # 测试平台
    tester: 'peomeng'        # 测试责任人
   ```
4.2 **`run.py`文件重新指定配置参数**：可在`CI/CD`或本地执行时，通过`run.py`指定任意配置参数覆盖`config.yaml`
   ```text
    usage: run.py [-h] [--device_id DEVICE_ID] [--test_type TEST_TYPE] [--test_platform TEST_PLATFORM] [--tester TESTER]
    
    自动化测试参数(可选 默认读取config.yaml)
    
    options:
      -h, --help            show this help message and exit
      --device_id DEVICE_ID
                            设备号
      --test_type TEST_TYPE
                            测试类型
      --test_platform TEST_PLATFORM
                            测试平台
      --tester TESTER       测试人
   ```
4.3 **扩展配置参数**：可在`config.yaml`文件增加扩展的参数，并在`util\setter.py``util\read_parse.py`文件配置新增的参数，满足业务需求扩展。
   
## 5. 运行测试

   **通过主入口 `run.py` (推荐)**：该脚本封装了配置和激活虚拟环境，也可以指定配置参数。
   ```bash
   python run.py
   ```